...
int main() {
    ...
    for (i = 0; i < length; i++) 
    {    
        x[2*i]   =  i; //test1InputSamples[i][0];            // Real part
        x[2*i+1] =  0; //test1InputSamples[i][1];            // Imaginary part
    }

    // Perform the FFT
    //memcpy(x, codes, sizeof(codes));
